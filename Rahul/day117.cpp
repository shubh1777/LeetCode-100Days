...
public:
    ...
    array<int, 2> findmin2(vector<int>& row){
        ...
        for(int x: row){ // For each element in the row
            if (x<x0) {  // If the element is smaller than x0
                x1=exchange(x0, x); // Exchange the values of x0 and x, and assign the old value of x0 to x1
            //    x1=x0;
            //    x0=x;
            }
            else if(x<x1) x1=x; // If the element is not smaller than x0 but smaller than x1, assign it to x1
        }
    ...
    int minFallingPathSum(vector<vector<int>>& grid) {
        ...
        for(int i=1; i<n; i++){// main nested loop
